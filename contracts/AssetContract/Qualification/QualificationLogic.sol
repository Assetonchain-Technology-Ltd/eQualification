...
contract WorkerProfileLogic is QualificationDS,Roles {
    ...
    function addAttribute(bytes32 _key, bytes32 _org, bytes calldata _value, uint256 _datetime, string calldata _datatype) public 
    returns (bytes32)
    {
        ...
        require(Attributes[_key].createDate==0,"QL04");
    ...
    function removeAttribute(bytes32 _key,bytes32 _org,uint256 _datetime) public  {
        require(_orgAccessCheck(_org,msg.sender,TOKEN)||_orgAccessCheck(_org,msg.sender,OPERATOR)||_orgAccessCheck(_org,msg.sender,ADMIN) ,"QL09");
        require(attributecount>0 && Attributes[_key].createDate>0,"QL06");
    ...
    function removeEndorsement(bytes32 _key,uint256 _endorseNumber,bytes32 _org) public {
        ...
        require(attributecount>0 && Attributes[_key].createDate>0,"QL12");
    ...
    function addKeyStore(bytes memory _dercert,uint256 _datetime,bytes32 _org) public{
        require(_orgAccessCheck(_org,msg.sender,TOKEN)||_orgAccessCheck(_org,msg.sender,OPERATOR)||_orgAccessCheck(_org,msg.sender,ADMIN) ,"QL14");
        key.cert=_dercert;
        key.createDate=_datetime;
    }
    
    function delKeyStore(bytes32 _org) public{
        require(_orgAccessCheck(_org,msg.sender,TOKEN)||_orgAccessCheck(_org,msg.sender,OPERATOR)||_orgAccessCheck(_org,msg.sender,ADMIN) ,"QL14");
        delete key;
    }
